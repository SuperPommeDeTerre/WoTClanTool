...
var displayTanks = function(dataMyTanks, dataTankopedia) {
	...
	$.post('./server/player.php', {
		...
		for (var i=0; i<dataMyTanks.length; i++) {
			...
			tableContent += '<td data-value="' + (tankAdditionalInfos.is_ready?'1':'0') + '"><div class="togglebutton"><label>&nbsp;<input type="checkbox" class="chkTanksIsReady" id="chkTanksIsReady' + myTank.tank_id + '" value="' + myTank.tank_id + '"' + (myTank['is_ready']?' checked="checked"':'') + ' /></label></div></td>';
			tableContent += '</tr>';
